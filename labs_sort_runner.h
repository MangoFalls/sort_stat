...
struct SortData {
	...
	int check_sum{ 0 };	   // контрольная сумма (всех элементов)
	int steps{ 0 };		   // счетчик проходов
};

class SortBase {
protected:
	void check_sum_control(const std::vector<int>& arr, int check_sum, const char* fn);
...
public:
	SortRunner(std::vector<int> arr, SortBase* sofu);
